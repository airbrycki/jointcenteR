...
#'
#' @param dict_url The URL of the data dictionary .csv file that corresponds to the df year.
#' 
#' @return Dataframe with ACS factor variables labeled.
#'
#' @note Labels are for standard ACS variables only. JCHS variables will need to be labeled separately.
#'
#' @examples
...
#' acs2023 |> filter(as.integer(ten) >2) |> group_by(ten) |> summarise(tot = sum(wgtp))
#' 
#' @export
add_ACSlabels <- function(df, dict_url) {
  ...
  dict <- read.csv(dict_url, sep = ",", header = FALSE) |>
    dplyr::rename(row_type = V1,
                  ...
                  value = V6,
                  label = V7) |>
    dplyr::filter(row_type == "VAL" & data_type == "C" & data_length <= 4 & label != "Puerto Rico" & label != "Puerto Rico/PR") |>
    dplyr::mutate(value = dplyr::if_else(value %in% c("b", "bb", "bbb", "bbbb"), NA_integer_, as.numeric(value))) |>
